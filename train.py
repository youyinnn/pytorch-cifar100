...
from utils import get_network, get_training_dataloader, get_test_dataloader, WarmUpLR, \
    most_recent_folder, most_recent_weights, last_epoch, best_acc_weights, get_device
...
def train(epoch):
    ...
    for batch_index, (images, labels) in enumerate(cifar100_training_loader):
        ...
        pbar.set_postfix(dict(
            epoch=epoch,
            trained_samples=str(batch_index * args.b + len(images)),
            total_samples=str(len(cifar100_training_loader.dataset)),
            loss=loss.item()
        ))
...
if __name__ == '__main__':
    ...
    pbar = tqdm(total=settings.EPOCH)
